...
//            "Timestamp",
//            "Order Reference Number", "Buy/Sell Indicator",
            "Shares", "Stock", "Price"// "Match Number"
    );
    ...
    public <T extends Message> void write(Iterable<T> messages) throws IOException {
        for (Message m : messages) {
            ...
            } else if (m instanceof TradeMessage) {
                write((TradeMessage) m);
            }
        }
    }

    public void write(AddOrderMessage m) throws IOException {
        csvPrinter.printRecord(m.getShares(), m.getStockSymbol(), m.getPrice());
    }

    public void write(TradeMessage m) throws IOException {
//        csvPrinter.printRecord(m.getShares(), m.getStockSymbol(), m.getPrice());
    }
